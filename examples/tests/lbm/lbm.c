	...
	*ptr = malloc( size );
	if( ! *ptr ) {
		/*printf( "LBM_allocateGrid: could not allocate %.1f MByte\n",
		        size / (1024.0*1024.0) );*/
		ocall(1,"LBM_allocateGrid: could not allocate Memory\n",45,NULL,0,0);
		exit( 1 );
	}
#ifndef SPEC
	/*printf( "LBM_allocateGrid: allocated %.1f MByte\n",
	        size / (1024.0*1024.0) );*/
	ocall(1,"LBM_allocateGrid: allocated\n",29,NULL,0,0);
#endif
	*ptr += margin;
}
...
void LBM_showGridStatistics( LBM_Grid grid ) {
	...
	SWEEP_END
	ocall(1,"Grid Statistics Trimmed\n",25,NULL,0,0);
...
ocall(1,"LBM_compareVelocityField Output\n",33,NULL,0,0);
//#ifdef SPEC
