...
double tanh(double x)
{
	edge_data_t pkgstr;
  double number;
  ocall(33,&x,sizeof(double),&pkgstr,sizeof(edge_data_t),&number);
  return number;
}
...
//
/* https://stackoverflow.com/questions/16182365/why-are-vtables-not-being-implemented-correctly-on-embedded-platform */
...
static void svm_group_classes(const svm_problem *prob, int *nr_class_ret, int **label_ret, int **start_ret, int **count_ret, int *perm)
{
  ...
  ocall(2,&nr_class,sizeof(int),NULL,0,0);
  ocall(1,"Reached Here1\n",15,NULL,0,0);
	int *start = Malloc(int,nr_class);
  if(start == NULL)
  {
    ocall(1,"Malloc failed\n",15,NULL,0,0);
  }
  else
  {
    ocall(1,"Reached Here2\n",15,NULL,0,0);
  }
	start[0] = 0;
  ocall(1,"Reached Here3\n",14,NULL,0,0);
	for(i=1;i<nr_class;i++)
  {
    ocall(1,"Reached Here4\n",14,NULL,0,0);
		start[i] = start[i-1]+count[i-1];
  }

  ocall(1,"Reached Here5\n",14,NULL,0,0);
	for(i=0;i<l;i++)
	{
...
void svm_cross_validation(const svm_problem *prob, const svm_parameter *param, int nr_fold, double *target)
{
	...
	{
		nr_fold = l;
		ocall(1,"WARNING: # folds > # data. Will use # folds = # data instead (i.e., leave-one-out cross validation)\n",103,NULL,0,0);
...
double svm_get_svr_probability(const svm_model *model)
{
	...
	{
		ocall(1,"Model doesn't contain information for SVR probability inference\n",66,NULL,0,0);
...
/*bool read_model_header(FILE *fp, svm_model* model)
{
	...
	while(1)
	{
		...
		else if(strcmp(cmd,"kernel_type")==0)
		{
			...
			{
				ocall(1,"unknown kernel function.\n",26,NULL,0,0);
