...
#if LARGE_MESSAGES
#define SIZEOF_DATA11 5
#define SIZEOF_DATA12 10
#define SIZEOF_DATA13 15
...
void EAPP_ENTRY eapp_entry()
{
	...
	for (i = 0; i < (sizeof STRING_VECTORS / sizeof (struct string_vector)); i++) {
		...
		{
				ocall(OCALL_PRINT_BUFFER, "SHA-256 done with error(string)", 33, NULL ,0,0);
				EAPP_RETURN(1);
		}
	}
	...
	int n=0;
	int max_limit=256;max_limit=max_limit;
  s_rand((unsigned long)&n, (unsigned long)sizeof(int));
	...
	test((uint8_t *)s, sz, NULL);

	for (i = 0; i < (sizeof vectors / sizeof (struct vector)) && 0; i++) {
		const struct vector *vector = &vectors[i];
		if (test(vector->input, vector->input_len, vector->output))
		{
			destruct_binary_messages();
			ocall(OCALL_PRINT_BUFFER, "SHA-256 done with (binary)", 28, NULL ,0,0);
			EAPP_RETURN(1);
		}
	}
