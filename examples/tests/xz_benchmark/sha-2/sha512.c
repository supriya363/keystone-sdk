...
static unsigned char *sortof_strdup(const unsigned char * str, int len) {
    unsigned char *newstr;
    check_malloc( newstr, unsigned char *, sizeof(char) * len );
...
void sha_process(sha512_state *md, const void* src, u32 inlen)
{
    ...
    unsigned char *in = sortof_strdup((const unsigned char *)src, inlen);
    unsigned char *freeme = in;
