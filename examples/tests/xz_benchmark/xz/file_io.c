...
io_open_src_real(file_pair *pair)
{
	...
	do {
		pair->src_fd = open((char *)(pair->src_name), flags, 0);
...
#else
		pair->dest_name = (char *)"(benchmark output)"; // never used
#endif // SPEC
...
io_write_buf(file_pair *pair, const uint8_t *buf, size_t size)
{
	...
	while (size > 0) {
		const ssize_t amount = write(pair->dest_fd, (unsigned char *)buf, size);
		if (amount == -1) {
			if (errno == EINTR) {
				if (user_abort)
					return true;
