...
error:
	message_fatal(_("Value of the option `%s' must be in the range "
				...
				name, min, max);
#if defined(SPEC)
        // In the error case it probably doesn't matter what's returned, but
        // let's at least make it consistent.
        return 0;
#endif
}
...
{
#if defined(SPEC)
        thousand = BROKEN; // Windows isn't POSIX.  Sometimes POSIX isn't POSIX
#else
	if (thousand == UNKNOWN) {
		bufs[slot][0] = '\0';
